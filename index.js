...
function declareVariable(position) {
  store[position]++
}

function clearVariable(position) {
  store[position]--
}

function assignVariable(a, b) {
  store[b]++
}
...
function getItemAtIndex(array, index) {
  let itemsAddress = store[array + 2]
  let scale = store[array + 1]
}

function appendItem(array, item) {
  let count = store[array]
  let scale = store[array + 1]
  if (count == scale) {
    if (scale == 32) {

    } else {
      let address = allocate(scale * 2)
      let itemsAddress = store[array + 2]
      let i = 0
      while (i < scale) {
        let x = store[itemsAddress + i]
        store[address + i] = x
        i++
      }
      store[address + i] = item
    }
  } else {
